...
export interface SessionFormProps {
  ...
  drillContext: 'warmup' | 'main' | 'integrated' | 'gym';
  setDrillContext: (v: 'warmup' | 'main' | 'integrated' | 'gym') => void;
  ...
  handleAddDrill: () => void;
  handleEditDrill: (drill: Exercise, context: 'warmup' | 'main' | 'gym') => void;
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-thin scrollbar-thumb-white/10">
            <form id="sessionForm" onSubmit={handleAddSession} className="space-y-8">
              ...
              {/* 2. Gym (Pre-Training) */}
              <Section title={t('sectionGym')} icon={Dumbbell} accentColor="bg-green-500">
                <div className="space-y-3">
                  {/* Existing gym exercises list */}
                  {newSession.gym.map((drill, idx) => (
                    <div key={drill.id} className="flex items-center gap-3 bg-surface-elevated px-4 py-2.5 rounded-lg border border-white/[0.06] group">
                      <span className="text-[10px] font-bold text-green-400 w-5 shrink-0">{idx + 1}.</span>
                      <span className="text-xs font-semibold text-on-surface flex-1 truncate">{drill.title}</span>
                      {drill.repetitions && (
                        <span className="text-[10px] font-bold text-on-surface-variant bg-white/[0.04] px-2 py-0.5 rounded">{drill.repetitions}</span>
                      )}
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                        <button type="button" onClick={() => {
                          setDrillContext('gym');
                          setCurrentDrill({ ...emptyDrill, type: 'gym', title: drill.title, repetitions: drill.repetitions });
                  ...
                  {/* Inline gym exercise form */}
                  {isAddingDrill && drillContext === 'gym' ? (
                    <div className="bg-surface-elevated rounded-xl border border-green-500/20 overflow-hidden">
                      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-white/[0.04] bg-green-500/5">
                        ...
                        <button type="button" onClick={handleCancelDrill} className="text-on-surface-variant hover:text-error transition-colors">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                      <div className="p-4 space-y-3">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={e => setCurrentDrill(prev => ({ ...prev, title: e.target.value }))}
                            className="w-full bg-surface border border-white/[0.06] rounded px-3 py-2 text-xs focus:border-green-500/40 outline-none transition-colors"
                            placeholder={t('gymExercisePlaceholder')}
                            autoFocus
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[9px] text-on-surface-variant uppercase font-label font-bold">{t('setsAndReps')}</label>
                          <div className="flex items-center gap-2">
                            <input
                              type="text"
                              value={(currentDrill.repetitions || '')}
                              onChange={e => setCurrentDrill(prev => ({ ...prev, repetitions: e.target.value }))}
                              className="w-full bg-surface border border-white/[0.06] rounded px-3 py-2 text-xs focus:border-green-500/40 outline-none transition-colors"
                              placeholder={t('setsAndRepsPlaceholder')}
                            />
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center justify-end gap-2 px-4 py-2.5 border-t border-white/[0.04]">
                        <button type="button" onClick={handleCancelDrill} className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest hover:bg-white/[0.03] rounded transition-all">{t('discard')}</button>
                        <button
                          type="button"
                          onClick={() => { if (currentDrill.title) handleAddDrill(); }}
                          className="bg-green-500 text-white px-5 py-1.5 rounded text-[10px] font-bold uppercase tracking-widest hover:bg-green-600 transition-all active:scale-95 shadow-lg shadow-green-500/20"
                        >
                          {editingDrillId ? t('saveChanges') : t('addExerciseToSession')}
                        </button>
                      </div>
                    </div>
                  ) : (
                    <button
                      type="button"
                      onClick={() => { setDrillContext('gym'); setCurrentDrill({ ...emptyDrill, type: 'gym' }); setEditingDrillId(null); setIsAddingDrill(true); }}
                      className="w-full flex items-center justify-center gap-2 p-3 rounded-lg border-2 border-dashed border-white/[0.08] hover:border-green-500/30 hover:bg-green-500/5 text-on-surface-variant hover:text-green-400 transition-all text-xs"
                    >
                      <Plus className="w-4 h-4" /> {t('createNewExercise')}
                    </button>
                  )}
                </div>
              </Section>
