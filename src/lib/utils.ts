...
/** Extract displayable image URL from a diagram field (supports JSON format and legacy base64). */
export function getDiagramImage(diagram: string | undefined): string | undefined {
  if (!diagram) return undefined;
  try {
    const parsed = JSON.parse(diagram);
    return parsed.image || undefined;
  } catch {
    return diagram.startsWith('data:image') ? diagram : undefined;
  }
}
